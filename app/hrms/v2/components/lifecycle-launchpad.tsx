...
import { useHRMSRole } from '@/app/hrms/v2/components/use-hrms-role';
          ...
          />
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <Link href="/hrms/v2/self-service" className="hrms-btn hrms-btn-primary px-4 py-2 text-sm">Open Self-Service Hub</Link>
                <Link href="/hrms/v2/self-service/pre-onboarding" className="hrms-btn hrms-btn-secondary px-4 py-2 text-sm">Open Pre-Onboarding</Link>
              </div>
            </section>
        </div>
      </div>
      ...
      <div className="hrms-main-with-nav">
        ...
        <section className="hrms-lifecycle-shell">
          <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            {items[0]?.href ? (
              <Link href={items[0].href} className="hrms-btn hrms-btn-primary px-4 py-2 text-sm">
                {items[0].cta || 'Start Workflow'}
              </Link>
            ) : null}
            {items[1]?.href ? (
              <Link href={items[1].href} className="hrms-btn hrms-btn-secondary px-4 py-2 text-sm">
              ...
              <div className="hrms-lifecycle-card-grid">
                {items.map((item, index) => (
                  <article key={item.title} className="hrms-lifecycle-card">
                    ...
                    {item.href ? (
                      <Link href={item.href} className="hrms-lifecycle-link">{item.cta || 'Open'}</Link>
                    ) : (
